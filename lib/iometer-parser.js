...
function parseStatus(data, meterNumber) {
	...
	if (device.id !== undefined)                    result.push({ id: `device-${meterNumber}.id`,                val: device.id });
	if (device.bridge?.rssi !== undefined)           result.push({ id: `device-${meterNumber}.bridge_rssi`,       val: device.bridge.rssi });
	if (device.bridge?.version !== undefined)        result.push({ id: `device-${meterNumber}.bridge_firmware`,   val: device.bridge.version });
	if (device.core?.rssi !== undefined)             result.push({ id: `device-${meterNumber}.core_rssi`,         val: device.core.rssi });
	if (device.core?.version !== undefined)          result.push({ id: `device-${meterNumber}.core_firmware`,     val: device.core.version });
	if (device.core?.batteryLevel !== undefined)     result.push({ id: `device-${meterNumber}.battery_level`,     val: device.core.batteryLevel });
	if (device.core?.powerStatus !== undefined)      result.push({ id: `device-${meterNumber}.power_status`,      val: device.core.powerStatus });
	if (device.core?.attachmentStatus !== undefined) result.push({ id: `device-${meterNumber}.attachment_status`, val: device.core.attachmentStatus });
	if (data.meter?.number !== undefined)            result.push({ id: `device-${meterNumber}.meter_number`,      val: data.meter.number });
